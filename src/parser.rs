...
"#;
        let tmp_dir = TempDir::new().expect("Couldn't create temporary directory");
        let file_path = tmp_dir.path().join("test.sv");
        std::fs::write(&file_path, source.as_bytes()).expect("Couldn't write source");

        dbg!(parse(&file_path).unwrap());
    }
}
